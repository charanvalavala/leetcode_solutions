...
class Solution {
    bool checkHorizontal(const vector<vector<int>>& grid) {
        ...
        vector<int> bot_inventory(100001, 0);
        for (int i = 0; i < m; i++)
            for (int j = 0; j < n; j++) {
                total_weight += grid[i][j];
                bot_inventory[grid[i][j]]++;
            }

        vector<int> top_inventory(100001, 0);
        long long top_weight = 0;
        long long bot_weight = total_weight;
        ...
        for (int i = 0; i < m - 1; i++) {
            ...
            if (diff > 0) {  
                ...
                if (is_1d) {
                    ...
                    if (top_inventory[d] > 0) return true;
                }

            } else {  
                bool single_cell = (i == m - 2 && n == 1);
                if (single_cell) continue;
                bool is_1d = (i == m - 2 || n == 1);
                if (is_1d) {
                    if (i == m-2 && (grid[m-1][0] == d || grid[m-1][n-1] == d)) return true;
                    if (n == 1  && (grid[i+1][0]  == d || grid[m-1][0]   == d)) return true;
                } else {
                    if (bot_inventory[d] > 0) return true;
                }
            }
        }
        return false;  
